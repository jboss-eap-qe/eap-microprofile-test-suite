...
public class DisablePropertiesTest {
     ...
     * skip doc generation for exposed "routed" JAX-RS endpoints as service consumers must rely on the original
     ...
     *
     * @return An {@link org.jboss.shrinkwrap.api.Archive} instance containing the Local Service Router application.
     */
    ...
    public static WebArchive localServicesRouterDeployment()
        ...
        return ShrinkWrap.create(
                ...
                .addClasses(
                        ...
                        OpenApiFilter.class)
                .addAsManifestResource(new StringAsset(CONFIGURATION_TEMPLATE + "\n" + props),
     ...
     *                {@code mp.openapi.extensions.enabled} (default = true) extension property - but rather it just
     *                disables class annotation scanning and processing the generated documentation programmatically.
     *                In this test case, the OpenAPI document is generated by processing the first deployment
     *                (Service Provider), despite it defining {@code mp.openapi.scan.disable=true}, hence an
     *                empty OpenAPI document is generated with defaults, e.g.: {@code title=Generated API}.
     *                When the second application - i.e. the Local Service Router - is deployed the OpenAPI generation
     *                kicks in again, starting from the static file, and passing through {@link OpenApiFilter} and
     *                {@link OpenApiModelReader} modifications.
     *                Specifically, {@link OpenApiModelReader} modifies {@code title}, which therefore becomes a
     *                discordant global property.
     * @tpPassCrit The server is logging expected {@code WFLYMPOAI0009} message because the first deployment does not
     *             deactivate OpenAPI subsystem, and when the second application is deployed the {@code title}
     *             property becomes discordant.
     * @tpSince EAP 7.4.0.CD19
     */
    @Test
    public void testWarningIsLoggedBecauseOfDiscordantTitleProperty() throws ConfigurationException, IOException {
        try (OnlineManagementClient client = ManagementClientProvider.onlineStandalone()) {
            final String message = "WFLYMPOAI0009: Ignoring deployment-specific property value for info.title due to " +
                    "conflicts: {localServicesRouterDeployment=LSR App, serviceProviderDeployment=Generated API}";
            ModelNodeLogChecker modelNodeLogChecker = new ModelNodeLogChecker(client, 100);
            Assert.assertTrue(modelNodeLogChecker.logContains(message));
        }
    }
}
